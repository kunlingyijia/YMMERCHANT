...
- (void)LQPhotoPicker_initPickerView{
    ...
    self.pickerCollectionView.backgroundColor = [UIColor yellowColor];
    ...
    self.pickerCollectionView.backgroundColor = [UIColor grayColor];
    ...
    //添加图片提示
    self.addImgStrLabel = [[UILabel alloc]initWithFrame:CGRectMake(([UIScreen mainScreen].bounds.size.width-100) /3+10, 50, 70, 20)];
    //self.addImgStrLabel.text = @"添加图片";
    self.addImgStrLabel.textColor = [UIColor colorWithRed:153.0/255.0 green:153.0/255.0 blue:153.0/255.0 alpha:1.0];
...
- (NSMutableArray *)LQPhotoPicker_selectedAssetArray {
    if (!_LQPhotoPicker_selectedAssetArray) {
        self.LQPhotoPicker_selectedAssetArray = [NSMutableArray arrayWithCapacity:0];
    }
    return _LQPhotoPicker_selectedAssetArray;
}
