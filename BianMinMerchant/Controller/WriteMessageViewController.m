...
@interface WriteMessageViewController ()<LQPhotoPickerViewDelegate,UIImagePickerControllerDelegate, UINavigationControllerDelegate,AMapLocationManagerDelegate>
//定位
...
- (void)initView {
    ...
    self.have24hourWater = 0;
    self.kindIndex = 0;
    UILabel *shopKind = [UILabel new];
    shopKind.text = @"分类";
    shopKind.textAlignment = NSTextAlignmentRight;
    shopKind.textColor = [UIColor colorWithHexString:@"#333333"];
    shopKind.font  = [UIFont systemFontOfSize:14];
    [self.container addSubview:shopKind];
    [shopKind mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.left.equalTo(self.container).with.offset(10);
        make.size.mas_equalTo(CGSizeMake(60, 20));
    }];
    DWHelper *helper = [DWHelper shareHelper];
    if (helper.shopType == 2) {
        self.kindBgview = [[ShopKindView alloc] initWithKindArr:self.bmChild];
        self.kindBgview.backgroundColor = [UIColor yellowColor];
        [self.container addSubview:self.kindBgview];
        
        [self.kindBgview mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.container).with.offset(5);
            make.right.equalTo(self.container);
            make.left.equalTo(shopKind.mas_right);
            NSInteger kindCount = self.bmChild.count/3;
            NSInteger indexCount = self.bmChild.count % 3;
            if (self.bmChild.count == 0) {
                make.height.mas_equalTo(@(0));
            }else if(self.bmChild.count < 4) {
                make.height.mas_equalTo(@(30));
            }else if (indexCount == 0) {
                make.height.mas_equalTo(@(30*kindCount));
            }else if (indexCount != 0) {
                make.height.mas_equalTo(@(30*(kindCount + 1)));
            }else {
                make.height.mas_equalTo(@(0));
            }
        }];
        ...
        self.kindBgview = [[ShopKindView alloc] initWithKindArr:self.kindArr];
        [self.container addSubview:self.kindBgview];
        ...
        [self.kindBgview mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.right.equalTo(self.container);
            make.left.equalTo(shopKind.mas_right);
            NSInteger kindCount = self.kindArr.count/3;
            NSInteger indexCount = self.kindArr.count % 3;
            if (self.kindArr.count == 0) {
                make.height.mas_equalTo(@(0));
            }else if(self.kindArr.count < 4) {
                make.height.mas_equalTo(@(30));
            }else if (indexCount == 0) {
                make.height.mas_equalTo(@(30*kindCount));
            }else if (indexCount != 0) {
    ...
    [self.kindBgview layoutIfNeeded];
    UILabel *firstShow = [UILabel new];
    firstShow.text = @"*必选";
    firstShow.textColor = [UIColor colorWithHexString:@"#aaaaaa"];
    firstShow.font  = [UIFont systemFontOfSize:12];
    [self.container addSubview:firstShow];
    [firstShow mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.kindBgview);
        make.top.equalTo(self.kindBgview.mas_bottom);
        make.size.mas_equalTo(CGSizeMake(60, 20));
    }];
    [firstShow layoutIfNeeded];
    ...
    [firstLine mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(firstShow.mas_bottom).with.offset(5);
        make.right.equalTo(self.container);
        make.height.mas_equalTo(@(1));
    }];
    [firstLine layoutIfNeeded];
    
    UILabel *shopGroup = [UILabel new];
    shopGroup.text = @"商圈";
    shopGroup.textAlignment = NSTextAlignmentRight;
    shopGroup.textColor = [UIColor colorWithHexString:@"#333333"];
    shopGroup.font  = [UIFont systemFontOfSize:14];
    [self.container addSubview:shopGroup];
    [shopGroup mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.container).with.offset(10);
        make.top.equalTo(firstLine).with.offset(10);
        make.size.mas_equalTo(CGSizeMake(60, 20));
    }];
    [shopGroup layoutIfNeeded];
    
    
    self.groupBgview = [[ShopGroupView alloc] initWithGroupArr:self.groupArr];
    [self.container addSubview:self.groupBgview];
    [self.groupBgview mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(firstLine).with.offset(5);
        make.right.equalTo(self.container);
        make.left.equalTo(shopGroup.mas_right);
        make.height.mas_equalTo(@(60));
    }];
    [self.groupBgview layoutIfNeeded];
    
    UILabel *secondShow = [UILabel new];
    secondShow.text = @"*非必选,不可多选";
    secondShow.textColor = [UIColor colorWithHexString:@"#aaaaaa"];
    secondShow.font  = [UIFont systemFontOfSize:12];
    [self.container addSubview:secondShow];
    [secondShow mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.groupBgview);
        make.top.equalTo(self.groupBgview.mas_bottom);
        make.size.mas_equalTo(CGSizeMake(100, 20));
    }];
    [secondShow layoutIfNeeded];
    ...
    [secondLine mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.container).with.offset(10);
        make.top.equalTo(secondShow.mas_bottom).with.offset(5);
        make.right.equalTo(self.container);
        make.height.mas_equalTo(@(1));
    }];
    ...
    [shopLogo mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(secondLine).with.offset(15);
        make.left.equalTo(self.container).with.offset(10);
        make.size.mas_equalTo(CGSizeMake(60, 20));
    }];
    ...
    [self.selectedLogo mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(shopLogo);
        make.left.equalTo(shopLogo.mas_right).with.offset(5);
    ...
    [self.logoImage mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self.selectedLogo.mas_bottom).with.offset(20);
        make.size.mas_equalTo(CGSizeMake(100, 100));
    }];
    
    [self.logoImage layoutIfNeeded];
    self.shopImageArr = [[UILabel alloc] initWithFrame:CGRectMake(10, self.logoImage.frame.origin.y+350+20, 60, 20)];
    ...
    [self lLQPhotoPicker_updatePickerViewFrameY:self.logoImage.frame.origin.y+350+20];
    ...
    [foruLine mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.right.left.equalTo(self.bottomBgview);
        make.height.mas_equalTo(@(1));
    }];
    ...
    [self.longText mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(self.bottomBgview).with.offset(-10);
    ...
    [lonShow mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.longText.mas_bottom);
        make.left.right.equalTo(self.longText);
    ...
    [latShow mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.latText.mas_bottom);
        make.left.right.equalTo(self.latText);
        make.height.mas_equalTo(@(20));
    }];
...
- (void)settingMessage:(NSMutableArray *)arr {
    ...
    info.merchantCategoryId = model.merchantCategoryId;
